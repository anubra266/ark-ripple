...
describe('Tabs', () => {
  ...
  it('should not loop focus if loopFocus is false', async () => {
  ...
  it('should render list with data-part="list"', async () => {
    const { container } = render(ComponentUnderTest);
    expect(container.querySelector('[data-part="list"]')).toBeInTheDocument();
  });

  it('should render triggers with data-part="trigger"', async () => {
    const { container } = render(ComponentUnderTest);
    const triggers = container.querySelectorAll('[data-part="trigger"]');
    expect(triggers.length).toBe(4);
  });

  it('should render content with data-part="content"', async () => {
    const { container } = render(ComponentUnderTest);
