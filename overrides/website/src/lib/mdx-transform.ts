...
function readExampleFile(framework: string, component: string, id: string): string | null {
  ...
  try {
    ...
    return content.replaceAll(/from '\.\/icons'/g, `from 'lucide-ripple'`).replace(/.*@ts-expect-error.*\n/g, '')
  } catch {
    return null
  }
}
...
export function replaceComponentTypes(text: string): string {
  ...
  for (const match of matches) {
    ...
    for (const framework of typesFrameworks) {
