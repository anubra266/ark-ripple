...
To render a menu item as a link, render the link as the menu item itself using `asChild`, not as a child of the
menu item.
...
export component MenuItemLink({ href, target, children, ...rest }: MenuItemLinkProps) {
  return (
    <Menu.Item>
      component asChild({ propsFn }) {
        <a {...propsFn({...rest, href, target })}>
          <@children />
        </a>
      }
    </Menu.Item>
